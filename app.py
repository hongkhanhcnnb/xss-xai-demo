...
from model_utils import XSSPredictor, extract_structural_features
...
st.markdown(
    ...
    <style>
    .main-title {
        font-size: 2.2rem; font-weight: 800; margin-bottom: 0.2rem;
    }
    .subtitle {
        color: #667085; font-size: 1.05rem; margin-bottom: 1.2rem;
    }
    .risk-card {
        padding: 1.2rem; border-radius: 18px; border: 1px solid #e5e7eb;
...
st.markdown('<div class="main-title">XMD-XSS: Demo phát hiện payload XSS bằng học sâu đa phương thức</div>', unsafe_allow_html=True)
st.markdown(
...
with st.sidebar:
    ...
    use_example = st.button("Đưa ví dụ vào ô nhập")

predictor = load_predictor(str(artifact_path))

default_payload = EXAMPLES[selected_example] if use_example else '<script>alert(1)</script>'
...
with col_b:
    st.caption("Lưu ý: Đây là website demo học thuật cho KLTN, không thay thế hệ thống WAF/IDS thực tế.")

if analyze or payload:
    ...
    st.progress(min(max(prob_xss, 0.0), 1.0), text=f"P(XSS) = {prob_xss:.4f}")

    left, right = st.columns([1, 1])

    with left:
        st.subheader("Giải thích nhanh")
        st.write("Các tín hiệu dưới đây kết hợp giữa dấu hiệu bảo mật trong payload và thống kê XAI toàn cục đã sinh từ notebook.")
        st.dataframe(result["top_local_signals"], use_container_width=True, hide_index=True)

        st.subheader("Token hóa payload")
        token_df = pd.DataFrame({"STT": range(1, len(result["tokens"]) + 1), "Token": result["tokens"]})
        st.dataframe(token_df, use_container_width=True, hide_index=True)

    with right:
        st.subheader("Đặc trưng cấu trúc nổi bật")
        feature_df = pd.DataFrame(
            [{"Feature": k, "Value": v} for k, v in result["structural_features"].items()]
        )
        feature_df = feature_df[feature_df["Value"].astype(float).abs() > 0].head(30)
        st.dataframe(feature_df, use_container_width=True, hide_index=True)

        st.subheader("Xác suất hai lớp")
        chart_df = pd.DataFrame(
            {"Lớp": ["Benign", "XSS"], "Xác suất": [result["prob_benign"], result["prob_xss"]]}
        )
...
tab1, tab2, tab3, tab4 = st.tabs(["So sánh mô hình", "Confusion matrix", "XAI toàn cục", "Thông tin dataset"])
...
with tab3:
    ...
    with xai_cols[0]:
        ...
        if not predictor.global_ig.empty:
            st.dataframe(predictor.global_ig.head(30), use_container_width=True, hide_index=True)
        else:
    ...
    with xai_cols[1]:
        ...
        if not predictor.global_struct.empty:
            st.dataframe(predictor.global_struct.head(30), use_container_width=True, hide_index=True)
        else:
...
with tab4:
    if not predictor.dataset_summary.empty:
        ...
        st.dataframe(predictor.dataset_summary, use_container_width=True, hide_index=True)
    if not predictor.label_distribution.empty:
        st.write("Label distribution")
        st.dataframe(predictor.label_distribution, use_container_width=True, hide_index=True)
